...
import { fetchGetListDropdownCategoryPost } from "@/api/categoryPost";
...
export default {
  ...
  created() {
    ...
    this.fetchGetListDropdownCategoryPost();
  },
  methods: {
    fetchData(id) {
      this.loading = true;
    ...
    fetchGetListDropdownCategoryPost() {
      fetchGetListDropdownCategoryPost()
        .then(response => {
          this.categoryListOptions = response;
        })
        .catch(err => {
          console.log(err);
        });
